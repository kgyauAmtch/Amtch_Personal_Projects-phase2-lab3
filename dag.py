...
# The schema you expect in every incoming CSV (order-agnostic)
EXPECTED_COLUMNS = {
    "user_id",
    "track_id",
    "listen_time"
...
def file_already_archived(file_name: str) -> bool:
    """Check if *any* date sub-folder under ARCHIVE_PREFIX already has file_name."""
    s3 = boto3.client("s3", region_name=AWS_REGION)
...
# ── DAG definition ─────────────────────────────────────────────────────
default_args = {"owner": "data-engineering", "retries": 2, "depends_on_past": False}
...
with DAG(
    ...
    #  Branch on “already processed?”
    def branch_on_processed(ti, **_):
        key = ti.xcom_pull(task_ids="detect_latest_file", key="stream_file_key")
        file_name = key.split("/")[-1]
        return "end_stream" if file_already_archived(file_name) else "run_clean_and_transform_glue_job"
    branch_if_processed = BranchPythonOperator(
        task_id="branch_if_processed",
        python_callable=branch_on_processed,
    )

    # Normal ETL path (unchanged)
    run_clean_glue_job = GlueJobOperator(
        task_id="run_clean_and_transform_glue_job",
        job_name=GLUE_CLEAN_JOB,
        aws_conn_id="aws_default",
        region_name=AWS_REGION,
        iam_role_name="lab3-gluerole",
    ...
    #End-of-branch placeholder
    end_stream = EmptyOperator(task_id="end_stream")

    # ── Dependencies ────────────────────────────────────────────────────
    wait_for_stream_file >> detect_latest_file >> validate_columns >> branch_if_processed
    branch_if_processed >> end_stream                           # if already processed
    branch_if_processed >> run_clean_glue_job                   # fresh file path
    run_clean_glue_job >> run_kpi_glue_job >> archive_stream_data >> end_stream
